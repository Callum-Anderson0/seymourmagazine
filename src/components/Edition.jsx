...
function Edition({ 
  ...
  return (
    <div className={cardClasses}>
      <div className="edition-container my-auto p-0 md:p-8 flex flex-col md:flex-row md:items-stretch gap-6 items-center justify-center">
        {/* Image first on small, right side on large */}
        <div className="w-full h-full md:w-1/2 order-1 md:order-2 flex my-auto justify-center items-center md:h-full">
          <FadeContent>
            <img
              src={imageSrc}
              alt={imageAlt}
              className="w-full rounded transform scale-100 z-10 object-cover md:h-full"
            />
          </FadeContent>
        </div>

        {/* Text content */}
        <div className={`order-2 md:order-1 md:w-1/2 md:mt-8 flex flex-col justify-between items-stretch md:gap-4 h-full font-medium ${textClasses}`}>
          <FadeContent className="flex justify-center" delay={0}>
            <h2 className={`text-2xl font-bold text-centre ${titleClasses}`}>
              {title}
            </h2>
          </FadeContent>
          <FadeContent className="" delay={0}>
            <p className={`text-justify ${textClasses}`}>
              {description}
            </p>
          </FadeContent>
